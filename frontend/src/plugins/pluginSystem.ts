...
export const PluginSystemProvider = ({ children }: { children: React.ReactNode }) => {
  return React.createElement(PluginSystemContext.Provider, { value: pluginSystem }, children);
};
...
export const withPluginSystem = <P extends object>(Component: React.ComponentType<P>): React.FC<P> => {
  return (props: P) => {
    ...
    // 应用afterRender钩子
    component = pluginSystem.applyHooks<React.ReactNode>('component:afterRender', component, props);
