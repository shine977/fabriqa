    ...
    const handleValidateRequest = () => {
      ...
      fields.forEach(field => {
        if (field.type === 'number' && values[field.name]) {
          formattedData[field.name] = Number(values[field.name]);
        }
        // 处理布尔类型
        else if (field.type === 'checkbox' || field.type === 'switch') {
          formattedData[field.name] = Boolean(values[field.name]);
        }
        // 处理嵌套对象 (假设使用点符号标记，如 meta.noCache)
        else if (field.name.includes('.')) {
          const [parentKey, childKey] = field.name.split('.');
          if (!formattedData[parentKey]) {
            formattedData[parentKey] = {};
          }
          if (field.type === 'checkbox' || field.type === 'switch') {
            formattedData[parentKey][childKey] = Boolean(values[field.name]);
          } else if (field.type === 'number') {
            formattedData[parentKey][childKey] = Number(values[field.name]);
          } else {
            formattedData[parentKey][childKey] = values[field.name];
          }
        }
      });
      ...
      const resultEvent = new CustomEvent('form:validationResult', {
        detail: {
          isValid,
          data: isValid ? formattedData : null
        }
      });
